/**
 * fast-check arbitraries for Rekono domain objects (web workspace).
 * Mirror the shapes defined in backend/src/schemas/.
 */
...
export const gstinArbitrary = fc
  .tuple(
    ...
    fc.array(fc.constantFrom(...'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')), { minLength: 5, maxLength: 5 }).map((a) => a.join('')),
  ...
  .map(([state, pan, seq, checkLetter, checkAlpha, lastChar]) => `${state}${pan}${seq}${checkLetter}${checkAlpha}Z${lastChar}`);

/** Positive monetary amount (paise-level precision, max ₹50L) */
export const positiveAmountArbitrary = fc.float({ min: 0.01, max: 5_000_000, noNaN: true }).map((n) => Math.round(n * 100) / 100);

/** Valid GST rates used in India */
export const gstRateArbitrary = fc.constantFrom(0, 5, 12, 18, 28);

/** Payment methods */
export const paymentMethodArbitrary = fc.constantFrom('PhonePe', 'Google Pay', 'Paytm', 'BHIM', 'cash', 'bank_transfer', 'other');
...
/** MongoDB-like ObjectId string (24 hex chars) */
export const objectIdArbitrary: fc.Arbitrary<string> = fc
